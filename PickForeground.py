...
class ImageProc(object):
    ...
    def __init__(self, frame_info = None, odd_index = None, thres = None, data = None,
                 ...
                 frame_size = (1920, 1080), ctu_size = (32,32), fps = 24, imgpath_prefix = 'test1/'):
        ...
        self.params['sds'] = sds
        self.params['size'] = frame_size
        self.params['fps'] = fps
        self.params['imgpath_prefix'] = imgpath_prefix
        self.params['ctu_info'] = ctu_size
    ...
    def showPictures(self, tag):
        ...
        for frIndex in range(frame_info['num_frame']):
            imgpath = self.params['imgpath_prefix'] + str(frIndex + 1).zfill(5) + ".jpg"
            img = np.array(Image.open(imgpath))
            for i in range(frame_info['H']):
                for j in range(frame_info['W']):
                    if tag[frIndex][i][j] == 99:
                        for a in range(ctu_info[0]):
                            for b in range(ctu_info[1]):
                                if i * ctu_info[0] + a >= img.shape[0] or j * ctu_info[1] + b >= img.shape[1]:
                                    continue
                                img[i * ctu_info[0] + a, j * ctu_info[1] + b, 0] = img[i * ctu_info[0] + a, j * ctu_info[1] + b, 0] * 0.5 + 255 * 0.5
            video.write(img)
        video.release()
